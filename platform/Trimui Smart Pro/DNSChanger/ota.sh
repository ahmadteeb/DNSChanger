...
# Main flow
$INFOSCREEN -m "Checking for update for DNSChanger"
...
if [[ "$local_version" == "$latest_version" ]]; then
    ...
    $INFOSCREEN -m "New update available: $latest_version" -t 0.2
    $INFOSCREEN -m "Please wait, this may take a few moments..."
    download_latest_release "$latest_version"
    clean_local_files
    extract_new_version
    update_version_file
    $INFOSCREEN -m "DNSChanger update complete."
fi